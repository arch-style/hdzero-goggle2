...
SD カード直下の `HDZGOGGLE.log` (今回の起動) と `HDZGOGGLE.1.log`〜`HDZGOGGLE.9.log`
(古い順に 9 回前まで)。**10 回分**残る。純正と、このブランチの 2026-09-10 より前は
`HDZGOGGLE.prev.log` の 1 つだけで、**電源を 2 回入れると消えていた** — 「あの起動の
ログが要る」と分かるのは常に後からなので、それでは間に合わない。

10 本で約 1MB。録画は実測 2.86MB/秒 (94.6MB で 33.1 秒) なので、**10 本全部で
録画 0.35 秒分**。59GB のカードは録画 6 時間ぶんで、そこにログ 10 本を置いても
残りは 5 時間 59.6 分。
