...
### 1.1 I2C バスが 5 秒止まる（相手は特定、原因は推定）
...
**対処 (未検証):** `Fixes` ページに **Short I2C Timeout** を追加した (既定 OFF)。
アダプタのタイムアウトを 500ms にするだけで、復帰の手順は変わらない。
**次の実機テストで、5006ms が 500ms 台になるかを確認する。**

- 500ms で足りるか: ここで一番長い転送でも数百ビット、最低速の 200kHz でも 1ms
- 取りこぼし自体が減るわけではない。頻度の原因 (1MHz 運用、burst 長) は別問題
- A/B するなら `Burst Tuner Writes` を OFF にした状態と比較する
