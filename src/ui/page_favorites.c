...
// Slot currently being edited with the dial, or -1 when no slot is focused.
static int editing_slot = -1;
...
static void hint_label_update(void) {
    ...
    if (editing_slot >= 0)
        snprintf(buf, sizeof(buf), "%s", _lang("Turn the dial to pick a channel, click to confirm"));
...
static lv_obj_t *page_favorites_create(lv_obj_t *parent, panel_arr_t *arr) {
    ...
    lv_obj_set_grid_cell(hint_label, LV_GRID_ALIGN_START, 1, 4, LV_GRID_ALIGN_START, row + 1, 1);
...
static void favorites_slot_save(int slot) {
    char key[8];
...
static void on_roller(uint8_t key) {
    ...
    uint8_t ch = g_setting.favorites.channel[editing_slot];
    ...
    g_setting.favorites.channel[editing_slot] = ch;
    slot_label_update(editing_slot);
}
...
static void on_click(uint8_t key, int sel) {
    ...
    if ((sel < ROW_SLOT_FIRST) || (sel > ROW_SLOT_LAST))
        return;

    int slot = sel - ROW_SLOT_FIRST;

    if (editing_slot == slot) {
        favorites_slot_save(slot);
        editing_slot = -1;
